...
guard let sRGB = CGColorSpace(name: CGColorSpace.sRGB) else {
    fatalError("Could not create the sRGB color space")
}
...
func rgb(_ hex: UInt32) -> CGColor {
    ...
    guard let color = CGColor(colorSpace: sRGB, components: [r, g, b, 1]) else {
        fatalError("Could not create a color for \(hex)")
    }
    return color
}
...
func fillGradient(_ context: CGContext, top: UInt32, bottom: UInt32) {
    guard let gradient = CGGradient(
        ...
        fatalError("Could not create the field gradient")
    }
...
/// The geometry of one stroked arc, so the drawing helper stays readable.
struct ArcSpec {
    let center: CGPoint
    let radius: CGFloat
    let start: CGFloat
...
func strokeGradientArc(
    ...
    context.setLineWidth(spec.width)
    guard let a = from.components, let b = to.components else {
        fatalError("Arc colors must be in an RGB color space")
    }
    for index in 0..<steps {
        let t0 = CGFloat(index) / CGFloat(steps)
        let t1 = CGFloat(index + 1) / CGFloat(steps)
        let angle0 = spec.start + (spec.end - spec.start) * t0
        let angle1 = spec.start + (spec.end - spec.start) * t1
        let p0 = CGPoint(x: spec.center.x + spec.radius * cos(angle0), y: spec.center.y + spec.radius * sin(angle0))
        let p1 = CGPoint(x: spec.center.x + spec.radius * cos(angle1), y: spec.center.y + spec.radius * sin(angle1))
        let r = a[0] + (b[0] - a[0]) * t0
        let g = a[1] + (b[1] - a[1]) * t0
        let bl = a[2] + (b[2] - a[2]) * t0
        guard let color = CGColor(colorSpace: sRGB, components: [r, g, bl, 1]) else {
            fatalError("Could not create a gradient step color")
        }
...
func write(_ context: CGContext, name: String) {
    ...
    guard let destination = CGImageDestinationCreateWithURL(
        ...
        fatalError("Could not create an image destination for \(name)")
    }
...
do {
    ...
    strokeRoundedRect(
        context,
        rect: CGRect(x: 202, y: 202, width: 620, height: 620),
        radius: 150, width: widths, color: rgb(0x2FB3CB)
    )
    strokeRoundedRect(
        context,
        rect: CGRect(x: 302, y: 302, width: 420, height: 420),
        radius: 105, width: widths, color: rgb(0x7FD9E8)
    )
    strokeRoundedRect(
        context,
        rect: CGRect(x: 402, y: 402, width: 220, height: 220),
        radius: 56, width: widths, color: rgb(0xF2F7F8)
    )
...
do {
    ...
    strokeLine(
        context,
        [CGPoint(x: 250, y: 358), CGPoint(x: 512, y: 620), CGPoint(x: 774, y: 358)],
        width: 116, color: rgb(0xF4F1EA)
    )
    strokeLine(
        context,
        [CGPoint(x: 420, y: 358), CGPoint(x: 512, y: 450), CGPoint(x: 604, y: 358)],
        width: 58, color: rgb(0x3FC6DE)
    )
